...
As expected, the results were inferior (average loss of accuracy around 10-20\%) to those obtained with CrossEntropyLoss, confirming the latter's superiority over the former in this context, particularly due to solving the vanishing gradient problem.
...
The Adam configuration reaches approximately 85\% accuracy by epoch 10, while the SGD configuration lags behind, only reaching around 75\% accuracy by the same epoch. 
...
The F1 score is around 0.80 for both configurations, with Adam slightly outperforming SGD with Momentum. 
...
\noindent
Finally, Figure~\ref{fig:cnn_cm_comparison} shows how this translates into the model's classification performance.
