...
\newpage
\section{Results}
In this section we first present and analyze the results obtained with each
type of deep learning architecture: MLNN and CNN. Then we proceed to a
...
The confusion matrix in Figure~\ref{fig:mlnn_test2_cm} shows that the
momentum-based SGD optimizer is a bit less stable in comparison to Adam
because, for exmaple, it tends to classify more samples as class 3. This could
be due to the optimizer's inability to adapt learning rates for individual
parameters, leading to suboptimal updates. In contrast, the Adam optimizer
(Figure~\ref{fig:mlnn_test1_cm}) demonstrates a more balanced classification
across classes, with fewer misclassifications. This further emphasizes Adam's
effectiveness across several metrics. \newline \newline The F1 score, is around
0.80 for both configurations, with Adam slightly outperforming SGD with
Momentum. This indicates that while both models are effective, and for simple
tasks like this one the differences are not very pronounced, Adam provides a
marginal advantage in balancing precision and recall (the thresholds are better
optimized).
...
\section{Conclusion}
This study compared the performance of Multi-Layer Neural Networks (MLNN) and
Convolutional Neural Networks (CNN) for blood cell classification using the BloodMNIST
dataset. As expected, the CNN architecture outperformed the MLNN across all evaluation
metrics, including accuracy, precision, recall, and F1-score. We were able not only to validate the
forementioned theoretical thesis but also do so in a practical, real world scenario with
all the complexities that entails (e.g.\ class imbalance)
